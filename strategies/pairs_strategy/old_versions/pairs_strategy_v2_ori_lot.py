...
class PairsStrategy(BaseStrategy):
    ...
    def init_strategy(self):
        ...
        # load params & recalibration at the beginning
        cur_datetime = max(
            self.engine.gateways[gateway_name].market_datetime
            for gateway_name in self.securities
        )
    ...
    def recalibration(self):
        ...
        for gateway_name in self.engine.gateways:
            ...
            for security_pair in self.security_pairs[gateway_name]:
                ...
                # Correlation
                logS1 = np.array([np.log(b.close * security1.lot_size) for b in
                                  self.ohlcv[gateway_name][security1]])
                logS2 = np.array([np.log(b.close * security2.lot_size) for b in
    ...
    def on_bar(self, cur_data: Dict[str, Dict[Security, Bar]]):
        ...
        for gateway_name in self.engine.gateways:
            ...
            for security_pair in self.security_pairs[gateway_name]:
                ...
                logS1 = np.log(
                    self.ohlcv[gateway_name][security1][-1].close
                    * security1.lot_size)
                logS2 = np.log(
                    self.ohlcv[gateway_name][security2][-1].close
                    * security2.lot_size)
        ...
        for gateway_name in self.engine.gateways:
            ...
            for security_pair in self.security_pairs[gateway_name]:
                ...
                spread_ma_short = ta.EMA(
                    spread, self.params['ma_short_length'])
                spread_ma_long = ta.EMA(
                    spread, self.params['ma_long_length'])
                spread_ma_zscore = (
                    spread_ma_short - spread_ma_long) / spread_ma_long.std()
    ...
    def load_opt_params(self, cur_datetime: datetime):
        if self.params["load_params"] != 1:
            return
        opt_params_files = os.listdir("opt_params")
        ...
        with open(f"opt_params/{opt_params_file}", "rb") as f:
        ...
        recalibration_lookback_ratio_mean = np.array(
            sorted(recalibration_lookback_ratios)[:]).mean()
        ma_short_length_mean = np.array(
            sorted(ma_short_lengths)[:]).mean()
        # if self.params["recalibration_lookback_ratio"] is not None:
        #     recalibration_lookback_ratio_mean = (
        #         0.4 * self.params["recalibration_lookback_ratio"]
        #         + 0.6 * recalibration_lookback_ratio_mean
        #     )
        # if self.params["ma_short_length"] is not None:
        #     ma_short_length_mean = (
        #         0.4 * self.params["ma_short_length"]
        #         + 0.6 * ma_short_length_mean
        #     )
