...
class PairsStrategy(BaseStrategy):
    ...
    def __init__(self,
        ...
        with open("strategies/pairs_strategy/params.json", "rb") as f:
            ...
            self.params["recalibration_interval"] = int(
                self.params["lookback_period"] * 0.05)
        ...
        for gateway_name in self.engine.gateways:
            ...
            if self.opt_params is None:
                ...
                shortlisted_security_pairs = [
                    ...
                    # if v['total_return'] > 0.0 and v['sharpe_ratio'] > 0.0
                    # if abs(v['maximum_drawdown']) < v['total_return']
                    # if v['total_return'] > 0.0
                ]
            ...
            for security_pair in self.security_pairs[gateway_name]:
                ...
                spread_ma_short = ta.EMA(spread, 20)
                spread_ma_long = ta.EMA(spread, 100)
                spread_ma_zscore = (
                    spread_ma_short - spread_ma_long) / spread_ma_long.std()


                sorted_spread = np.array(sorted(spread))
                n = list(sorted_spread > 0).index(True)
                entry_mul = st.norm.ppf(self.params['entry_threshold_pct'])
                exit_mul = st.norm.ppf(self.params['exit_threshold_pct'])

                positive_spread_mean = sorted_spread[n:].mean()
                positive_spread_std = sorted_spread[n:].std()
                positive_entry = positive_spread_mean + entry_mul * positive_spread_std
                positive_exit = positive_spread_mean + exit_mul * positive_spread_std

                negative_spread_mean = sorted_spread[:n].mean()
                negative_spread_std = sorted_spread[:n].std()
                negative_entry = negative_spread_mean - entry_mul * negative_spread_std
                negative_exit = negative_spread_mean - exit_mul * negative_spread_std

                long_entry_threshold = (negative_entry - spread_mean) / spread_std
                long_exit_threshold = (negative_exit - spread_mean) / spread_std
                short_entry_threshold = (positive_entry - spread_mean) / spread_std
                short_exit_threshold = (positive_exit - spread_mean) / spread_std

                prev_gamma = self.security_pairs_regression_params[
                    gateway_name][security_pair]["gamma"]
                self.security_pairs_regression_params[gateway_name][
                    security_pair]["gamma"] = gamma
                prev_mu = self.security_pairs_regression_params[
                    gateway_name][security_pair]["mu"]
                self.security_pairs_regression_params[gateway_name][
                    security_pair]["mu"] = mu
                self.security_pairs_regression_params[gateway_name][
                ...
                if (
                        ...
                        # and hurst_exponent(spread) < self.params['hurst_threshold']
                        # and half_life(spread) < self.params['half_life_threshold']
                ):
                    ...
                    # spread_ma_short = ta.EMA(spread, 20)
                    # spread_ma_long = ta.EMA(spread, 100)
                    # spread_ma_zscore = (spread_ma_short - spread_ma_long) / spread_ma_long.std()
    ...
    def on_bar(self, cur_data: Dict[str, Dict[Security, Bar]]):
        ...
        for gateway_name in self.engine.gateways:
            ...
            for security_pair in self.security_pairs[gateway_name]:
                ...
                gamma = self.security_pairs_regression_params[gateway_name][
                    security_pair]["gamma"]
                spread_mean = self.security_pairs_regression_params[gateway_name][
                    security_pair]["spread_mean"]
                spread_std = self.security_pairs_regression_params[gateway_name][
                    security_pair]["spread_std"]
                ...
                can_entry_long1_short2 = (
                    ...
                    # and (self.recalibration_date - cur_datetime).total_seconds() > self.params['recalibration_interval'] * TIME_STEP / 1000 * 0.25
                )
                ...
                entry_short1_long2 = (
                    cur_spread_zscore < short_entry_threshold < prev_spread_zscore
                )
                entry_long1_short2 = (
                    cur_spread_zscore > long_entry_threshold > prev_spread_zscore
                )
                ...
                exit_long1_short2 = (
                    cur_spread_zscore < long_exit_threshold < prev_spread_zscore
                )
                exit_short1_long2 = (
                    cur_spread_zscore > short_exit_threshold > prev_spread_zscore
                )
    ...
    def load_opt_params(self, cur_datetime: datetime):
        ...
        for i in range(1, len(opt_params_files)):
            ...
            _, _, _, prev_end_str = prev_opt_params_file.replace(".pkl", "").split("_")
            prev_end = datetime.strptime(prev_end_str, "%Y%m%d")
            _, _, _, cur_end_str = cur_opt_params_file.replace(".pkl", "").split("_")
