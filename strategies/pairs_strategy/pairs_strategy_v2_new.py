...
class PairsStrategy(BaseStrategy):
    ...
    def init_strategy(self):
        ...
        # load parameters and recalibrate at the beginning
        cur_datetime = max([
    ...
    def recalibration(self):
        ...
        for gateway_name in self.engine.gateways:
            ...
            #         if v['return_over_maximum_drawdown'] > 1.0
            #     ]
    ...
    def load_opt_params(self, cur_datetime: datetime):
        ...
        ma_short_lengths = [
            v['ma_short_length'] for k, v in opt_params.items()
            # if v['best_loss'] < 0
        ]

        tmp1 = np.array(recalibration_lookback_ratios)
        tmp2 = np.array(ma_short_lengths)

        # import matplotlib
        # matplotlib.use('TkAgg')
        # import matplotlib.pyplot as plt
        # plt.plot(tmp2, tmp1, "o")
        # plt.show()

        recalibration_lookback_ratio_mean = tmp1[1:-1].mean()
        ma_short_length_mean = tmp2[1:-1].mean()
        # recalibration_lookback_ratio_mean = np.array(
        #     sorted(recalibration_lookback_ratios)[1:-1]).mean()
        # ma_short_length_mean = np.array(
        #     sorted(ma_short_lengths)[1:-1]).mean()
        ...
        print(
            cur_datetime,
            round(recalibration_lookback_ratio_mean, 3),
            int(ma_short_length_mean)
        )
